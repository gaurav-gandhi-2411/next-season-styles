...
| C1 | **Done** 2026-09-23 (`96ec9d2`..`51d8c65`) | Model card `docs/MODEL_CARD.md`. Contracts (pandera) pass on the real data in 6 s, and corrupted inputs are rejected row by row. `nss.prod.cli train` from `configs/champion.yaml` reproduces the committed tables with **max \|diff\| 0.0** on all 11 metrics, coverage, Q_lo and Q_hi, bit-identical over 3 runs; artifact `20260923T172053Z-f513dc16` at `54a7ecd`, clean tree, registered and promoted as the initial champion. The registry refuses a guardrail-regressing challenger (test and CI fixture) and a dirty-tree artifact. The API smoke test passes locally and in Docker (same forecast to the digit). First CI run green: [35898008345](https://github.com/gaurav-gandhi-2411/next-season-styles/actions/runs/35898008345). |
| C2 | Not started (needs GG's one-time GCP sign-in) | — |
...
| 2026-09-23 | Phase C split into C1 (local, done) and C2 (GCP). Model work on H&M is complete. |
| 2026-09-23 | Data contracts use pandera (polars backend): vectorised and row-level; pydantic only for API bodies. `fashion_news_frequency` has both "NONE" and "None" in the source, accepted as-is. |
| 2026-09-23 | The API's `forecast` is the champion's **L2 point forecast** (the champion is L2 + CQR); the q50 quantile model was non-inferior but never adopted, so it is not served. The interval comes from q10/q90 + CQR. |
| 2026-09-23 | nss.prod imports lightgbm before anything loads pyarrow: with lightgbm 4.7.0 + pyarrow 18.1.0 on Windows, pyarrow first makes LightGBM training fault (native mechanism not identified). Guarded by a behavioural test with a negative control. |
| 2026-09-23 | Registry refuses artifacts trained from a dirty tree; the first champion needs --initial and passing reproduction checks. |
| 2026-09-23 | **Open, for GG before C2:** (1) the pre-stated coverage alert (4 weeks, [0.70, 0.90]) would have fired in 26 of 45 backtest windows (58%); it detects the real calibration lag but is too frequent to page on, so it needs retuning or reframing, and that should be pre-registered. (2) The Section 6 gate has no minimum effect size: a near-copy challenger can be refused for a negligible but significant guardrail drop. Decide whether to add an equivalence margin. |
| 2026-09-23 | CI runs 503 of the suite's tests; 42 modules needing the concept/image/LLM stack are excluded with reasons in tests/ci_light_excluded.txt and run locally only. |
