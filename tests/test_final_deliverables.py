...
# `final_deliverables.main()` is the earlier builder that reads the `is_selected` candidate table;
# the current final three (dress) were generated by `reselection_generate`, so that table has no
# dress row. The shipped figures are built by `final_selection_figures.main()` (covered by its
# own tests).
_LEGACY_F5_MAIN = pytest.mark.skip(reason="candidate table predates the current final three")
...
def test_build_margin_text_labels_gate1_and_diagnostic_band_separately() -> None:
    ...
    assert "[0.0300, 0.1000]" in text  # the real-space diagnostic band, printed
...
def test_main_writes_non_trivial_pngs(tmp_path: Path) -> None:
    """`main()` runs end-to-end against the repo's real artifacts and writes real PNGs."""
