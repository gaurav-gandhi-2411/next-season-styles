...
# `final_deliverables.main()` is the F5-era builder that reads F5's `is_selected` candidate table;
# the M-session final three (dress) were generated by `reselection_generate`, so that table has no dress
# row. The shipped figures are built by `final_selection_figures.main()` (covered by the H4 tests).
_LEGACY_F5_MAIN = pytest.mark.skip(reason="F5 candidate table predates the M-session final three")
