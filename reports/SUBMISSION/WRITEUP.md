...
**Neighbourhood features: tested, rejected.** Fifteen causal features on how a style's attribute-lattice neighbours are trending (`q1_decision.csv`), same embargo and frozen hyperparameters, adoption rule fixed in advance (paired top-20 CI must exclude zero): Hit@3-in-top20 0.528 → 0.611, diff +0.083, CI [−0.083, +0.250], so not adopted. The model does use them (15.9% of mean |SHAP|), so this is "no demonstrated gain on 12 origins", not "no effect". The numbers above stand.
...
|---|---|---|---|---|---|---|---|
| Sweater | pass | pass | pass | pass | pass | yes | Beige Melange Sweater: 17.8, rank 106/1,980, medium |
| Dress | pass | pass | pass | pass | pass | yes | Red Dresses Ladies: 11.0, rank 282, high |
| White top | pass | pass | **fail** (0.733 < 0.779; per-style 0.922 fails too) | pass (Florence advisory: fail) | fail (YN) | mostly (narrow cuffs) | Sport Jersey Fancy White: 15.8, rank 140, low |
| Bikini top (summer) | pass | pass | pass | fail (SmolVLM 0.28) | pass | yes | Orange All-over pattern: 37.9, rank 118/3,000, high |
...
**Closed loop, rebuilt as retrieval.** The first version had a small VLM caption the picture and mapped the words to a style; on 40 real photos it named the exact style 12.5% of the time, a specification error (garment group and department are not visible). The picture is now matched to the nearest real style by CLIP and DINOv2 similarity to that style's photos, with top-5 and a confidence label (`concept_forecast.py`, MCP `forecast_concept`, DEMO). Inside its index, leave-one-out over 159 photos names the exact style first 72.3%, top-5 86.8%, top-10 91.8% (chance 0.2/1.2/2.4%); top-1 is right 91% at high confidence, 11% at low. It is **a prototype**: the index holds 415–455 of the 1,980 styles (Kaggle stopped serving photos), so on the original 40 photos it scores 0/40, which measures coverage and is not comparable to 12.5%. Three of the four final concepts match their intended style, near-circularly, since the index holds their own references; the white top's intended style is second by 0.001. It is a forecast for the archetype the picture reads as, not for the design (`q2_retrieval_validation.csv`).
