...
to be `true` -- a deliberately strict, joint reading, same convention Gate 1 below reuses. **Since the
copy-anchor gate replaced it, `margin_band_pass` no longer gates `overall_pass`** -- it is still computed and reported on
...
## Input: a per-style `CopyCheckResult` (Gate 1, copy-anchor form)

```jsonc
{
  "clip_margin": 0.0948,
  "clip_copy_anchor_gen": 0.1359,          // this style's copy_anchor_gen CLIP mean
...
`margin_band_pass` used. `copy_check_pass(result, active_metrics={"clip"})` narrows this
...
provider pairings that happen to be reachable today. In this project's first QC run, every scored image
was generated by the `"local_sdxl"` backend (Gemini's own image-generation backend was quota-
blocked during generation), so this rule never actually excluded anything here -- it is
...
```jsonc
{
  ...
  "margin_band_pass": false,        // DIAGNOSTIC ONLY -- does NOT gate overall_pass
  "copy_check": {
  ...
  "overall_pass": false,            // copy_check_pass AND fidelity_pass
...
one available judge (as this project's first QC run did), kappa is **not
...
decision below, and is unchanged by the Gate 1 change.

## Gate 1: the sign-safe discount formula

**Why the old two-sided band was replaced.** `margin_band_pass` (the earlier gate) checked a
generated image's CLIP/DINOv2 margin against a band estimated from REAL H&M catalogue images
(6-23 real reference styles per embedding space) and cross-applied to a DIFFERENT distribution:
SDXL-generated images. The generated-space anchor calibration directly measured what the SAME margin metric looks like on
...
job is now done by Gate 2 for the "too dissimilar" question, AND (see below) as the
...
(`-0.0472`) -- and disagree in exactly the direction that matters:
...
`discount = 0.10` was chosen to match the original two-sided band's own implied 10% margin (the
starting point of `copy_anchor * 0.90`), generalized to stay correct under a sign flip.

**A second, independent finding, reported as an observation, not "fixed":** the
...
Sweater/CLIP combination, not something the gate-logic change resolves.

## Gate 1: dropping a non-discriminative metric

**The defect this fixes.** The original `unrelated_anchor_gen` (the "genuinely different garment"
...
even a deliberately-different-garment text prompt barely moved the embedding: the originally
measured pooled copy-vs-unrelated gap was CLIP `0.0029`, DINOv2 `0.0251` -- both near zero, meaning
...
**The fix.** `nss.generate.derive_unrelated_anchor_fix` regenerates ONLY
...
| Metric | copy_anchor_gen mean | unrelated_anchor_gen mean (original, contaminated) | unrelated_anchor_gen mean (corrected) | Gap (original, contaminated) | Gap (corrected) |
|--------|----------------------|-----------------------------------------------|---------------------------------------------|--------------------------|------------------------|
...
(`0.05`, chosen before this run, order-of-magnitude above both of the original contaminated gaps) by a wide
...
remained under the bar, `copy_check_pass(result, active_metrics=...)` (the generic mechanism,
...
## Gate 2: per-judge calibrated threshold (supersedes the flat 0.75)

**Why the flat 0.75 was replaced.** Originally, `fidelity_pass` was
...
**Measured resulting values, and which direction each moved (exact numbers, not
...
## Re-score of the 9 already-logged first-run attempts
...
gate does NOT retroactively pass any of the 9 logged attempts. Per style:
...
- **Sweater (0/3 pass):** all 3 attempts fail BOTH gates -- Gate 1 fails on CLIP specifically for
  ...
  misreads the known "degenerate fabric-texture close-up" defect as "knit fabric," not
...
opaque "out of band" verdict. It does NOT, however, resolve those failures "for free" -- the honest
...
## Re-score of the 24 already-generated `final_concepts_v2.csv` candidates
...
| Gate | Before (original gate) | After (corrected) |
|------|--------------------------|----------------------------|
...
`copy_anchor_gen` (the unrelated-anchor correction leaves this anchor untouched: it was ALREADY correct) and
`active_metrics` (BOTH metrics remain discriminative after the fix, so
`GATE1_ACTIVE_METRICS` stays `{"clip", "dinov2"}`, identical to the original default). Neither input
to Gate 1's actual pass/fail decision changed for these 24 rows -- the unrelated-anchor correction's contribution is
...
**Gate 2 moved substantially (0 -> 6/24) because the per-judge threshold correction changes the actual
...
## Gate 1, earlier version: the within-style MEDIAN benchmark (superseded by the p90 rule below)
...
(`nss.generate.within_style_benchmark`) -- never a pooled global figure (the pooled value 0.9401
...
judge reads attribute words, not garment integrity (the malformed cut-out and sheer-mesh
underwear candidates, seeds 42 and 44, scored 0.617 and 0.600 against a 0.513 threshold; the
0.68 once cited for the "folded object" was the lace original's score, and the folded
object, seed 45, was never scored by any judge before the repeated-call scoring). Visual inspection of every
candidate remains part of the procedure.

## CURRENT Gate 1: range-based within-style rule, p90 (supersedes the median)

**What was wrong with the median.** The earlier threshold was the *median* pairwise similarity between
...
**Effect.** Gate 1 on an earlier set of 12 candidates: median 4/12 -> p90 12/12; a later set of 4
underwear candidates: 4/4 under both; final selections: median 1/3 -> p90 3/3
(`gate1_rescored_within_style.csv`, `underwear_scored.csv`).

## CURRENT Gate 1b: nearest-reference check, calibrated on real articles
...
(each real reference's max cosine to the OTHER references, the same leave-one-out set as the control above), per
...
## CURRENT Gate 2 checklist: visually observable attributes only
...
on the same T-shirt image with the same checklist scored 0.425 where an earlier run had stored 0.6375 -- single
judge calls carry roughly +/-0.2 on one image; treat per-candidate fidelity as coarse.

## Gate 2 checklist, completed: non-visual pattern labels are excluded too

`garment_group` ("Jersey Basic") is dropped because it has no visual referent. The same defect
...
**Agent layer / MCP:** `agents/critic.md` and the MCP `score_concept` tool now run these
...
## Gate 2 fidelity is a median of repeated calls, with a stated noise bound

A single judge call is not a precise measurement: the same T-shirt image scored 0.6375 and then
0.425 across separate runs (~0.21). Each FINAL concept is therefore judged 3 times per judge
...
same-run repeats agreed to <= 0.006 (the T-shirt and sweater each returned three identical scores), so they show
repeatability, not accuracy -- they do not shrink the cross-run bound. A judge with 1..2 of 3
...
- **`copy_check_pass` requires BOTH metrics below their thresholds, deliberately stricter than a
  ...
  logic would paper over. This project's selection step used CLIP-primary ranking (documented in
...
Actual `nss.generate.concept_qc_pipeline` run output (2026-09-19) for style
`Ladieswear || T-shirt || Jersey Basic || Black || Solid`'s original (attempt 0) candidate
...
`GROQ_API_KEY`, confirmed by direct API test). Re-scored under the copy-anchor
gate (the honest re-score, `reports/tables/concept_qc_rescored_under_new_gate.csv`):
...
Actual run output for style `Ladieswear || Sweater || Knitwear || Beige || Melange`'s original
(attempt 0) candidate
(`data/generated/final_concepts/ladieswear_sweater_knitwear_beige_melange_seed42.png`). This is ALSO
the project's real negative-anchor edge case -- see "Gate 1: the sign-safe discount
...
This is the independently documented "degenerate fabric-texture close-up" quality problem
(`nss.generate.final_concepts` module docstring), and the blind judge's OWN
...
## The token budget can silently delete the design change
...
## Gate 3, the integrity floor and local judges
...
**Integrity: the GLOBAL floor gates, the per-style floor is advisory.** Both are
