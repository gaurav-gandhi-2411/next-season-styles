...
The top-k comparison with seasonal naive in the write-up (Hit@3-in-top20 +0.233, 95% interval [0.000, 0.567]) was measured at 4-week origin spacing: 12 origins, 10 of them with a seasonal-naive forecast. Measured at every week (48 origins, 42 with a seasonal-naive forecast; the same 12 models, block bootstrap with a 13-week block) it is **+0.095 [-0.040, +0.278]**, with an effective sample size of about 15. The 4-week grid was a favourable subsample: +0.233 on those 10 origins and +0.052 on the other 32. The qualitative claim in the submission holds. The model is comparable to seasonal naive on top-k and better on NDCG@10 (+0.130 [+0.080, +0.226]), Spearman (+0.200 [+0.192, +0.241]) and WMAPE (-0.041 [-0.055, -0.034]); a top-3 advantage over seasonal naive is not established. The submitted write-up is unchanged.

The final three concepts were chosen from a ranking by predicted growth (predicted intensity divided by the trailing 13-week mean intensity), and that ranking had not been validated. Tested on the 10 origins where the pipeline can produce a list at all, it beats a random ranking (Hit@3-in-top20 0.767 against 0.177) but not seasonal naive (0.800), and most of its skill is a base effect: a constant forecast, which can only rank styles by low recent intensity, scores 0.733. I then tried replacing the score with the model's forecast minus the seasonal-naive forecast. It removes the base effect (the constant forecast falls to the random floor) but fails the test I fixed beforehand: its Hit@3-in-top20 is 0.10 on the 10 origins (0.13 across 41 weekly origins), no better than random, against 0.80 (0.76) for seasonal naive's growth ranking. By the rule set before the test, the redesigned score is not adopted. The submitted concepts nevertheless remain selected by the growth ranking. The intensity ranking, which is the one that has been backtested, would have selected a black jersey T-shirt, a white knitwear sweater and grey trousers; none of the submitted three (beige melange sweater, red dress, white jersey top) would be chosen as the same style. Concepts were not regenerated because the submission had already been sent, and the submitted concepts, demo and write-up are unchanged. The finding itself is that realised growth on this dataset is largely mean reversion (Spearman -0.42 between trailing intensity and realised growth), so no growth target here is free of the base effect. The analysis, the pre-registered rules and the tables are in [`reports/v3/`](https://github.com/gaurav-gandhi-2411/next-season-styles/tree/feat/v3-improvements/reports/v3) on the branch `feat/v3-improvements`.
